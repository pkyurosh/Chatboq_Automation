...
class TestAccountSettings:
    ...
    def test_upload_account_photo(self):
        ...
        assert (
            new_src == old_src
        ), f"Logo src should not change after upload as its the same picture, still: {new_src}"
    ...
    def test_update_full_name(self):
        ...
        assert (
            self.account_name in error
        ), f"Expected '{self.account_name}' but got: {error}"
    ...
    def test_enter_phone_details(self):
        ...
        assert (
            "9812345678" in value or "981 2345678" in value
        ), f"Expected phone number but got: {value}"

    def test_enter_country(self):
        self.page.enter_country("Nepal")
        assert self.sb.is_text_visible(
            "Nepal"
        ), "Nepal should be visible after selection"
...
class TestAccountSettingsNegative:
    ...
    def test_invalid_phone_number(self, invalid_number, case_id):
        ...
        assert (
            "please enter a valid phone number" in error.lower()
