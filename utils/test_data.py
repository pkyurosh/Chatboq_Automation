...
def get_random_email():
    """
    Letters only + @test.com e.g. testabcdefgh@test.com
    New email every run so that the update button does not stay on disable state
    """
    random_str = "".join(random.choices(string.ascii_lowercase, k=8))
    return f"test{random_str}@test.com"
